...
def shuffle_list(ls_origin, SEED=None):
    if SEED is not None:
        random.seed(SEED)
    else:
        random.seed(time.time())

    random.shuffle(ls_origin)
    return ls_origin

def  shuffle_func(vid_item):
    vid_path = vid_item[0]
    vid_id = vid_item[1]
    if dataset == 'UCF101':
    ...
    for i, fname in enumerate(frame_names):
        with open(fname, 'rb') as f:
            data[i] = f.read()
    shuffled_names = shuffle_list(frame_names)
...
if __name__ == '__main__':
    ...
    pool.map(shuffle_func, zip(vid_list, range(len(vid_list))))
